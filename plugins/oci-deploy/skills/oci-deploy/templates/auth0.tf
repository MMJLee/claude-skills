...
# admin/user roles, post-login action that injects email/name/roles into JWT,
# and optional auto-assignment of admin role to one user.
#
# Requires:
#   - terraform/auth0 provider configured in main.tf
#   - AUTH0_DOMAIN, AUTH0_M2M_CLIENT_ID, AUTH0_M2M_CLIENT_SECRET set
#   - AUTH0_API_AUDIENCE, AUTH0_JWT_NAMESPACE, AUTH0_CALLBACK_URLS set
#
# After first apply, find your Auth0 user_id (Dashboard > Users) and set
# AUTH0_ADMIN_USER_ID in terraform.tfvars to auto-assign the admin role.
...
# --- Post-login action: require verified email, inject email/name/roles into JWT ---
# The namespace must match what your backend reads (e.g., your Go middleware
# pulls roles from <namespace>/roles).
